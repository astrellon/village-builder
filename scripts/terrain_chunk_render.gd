...
func create_cliff(surface_tool: SurfaceTool, face_index: int, face_lookup: PackedByteArray, x: int, y: int, face_type: TerrainChunkData.FaceType, dx: float, dy: float, p1: Vector3, p2: Vector3, to_cliff_height_1: float, to_cliff_height_2: float, type: int) -> int:
	if dx >= 0 && dy >= 0:
		return face_index
...
static func add_face_lookup(face_lookup: PackedByteArray, face_index: int, x: int, y: int, face_type: TerrainChunkData.FaceType, add_two: bool = false) -> int:
	...
	for y in range(terrain.size):
		...
		for x in range(terrain.size):
			...
			if is_flipped:
				...
				face_index = add_face_lookup(face_lookup, face_index, x, y, TerrainChunkData.FaceType.TYPE1)
				...
				face_index = add_face_lookup(face_lookup, face_index, x, y, TerrainChunkData.FaceType.TYPE2)
			else:
				uv_tl = Vector2(uv_x1, uv_y)
			...
			face_index = self.create_cliff(surface_tool, face_index, face_lookup, x, y, TerrainChunkData.FaceType.CLIFF_WEST, dx, dy, p_tl, p_bl, to_left.y, to_left.w, cliff_w)

			dx = to_right.z - heights.w
			dy = to_right.x - heights.y
			face_index = self.create_cliff(surface_tool, face_index, face_lookup, x, y, TerrainChunkData.FaceType.CLIFF_EAST, dx, dy, p_br, p_tr, to_right.z, to_right.x, cliff_e)

			dx = to_up.w - heights.y
			dy = to_up.z - heights.x
			face_index = self.create_cliff(surface_tool, face_index, face_lookup, x, y, TerrainChunkData.FaceType.CLIFF_NORTH, dx, dy, p_tr, p_tl, to_up.w, to_up.z, cliff_n)

			dx = to_down.x - heights.z
			dy = to_down.y - heights.w
			face_index = self.create_cliff(surface_tool, face_index, face_lookup, x, y, TerrainChunkData.FaceType.CLIFF_SOUTH, dx, dy, p_bl, p_br, to_down.x, to_down.y, cliff_s)
